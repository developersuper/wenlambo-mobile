  ...
  const makeRoundTrip = useCallback(
    async (fn, ...params) =>
      new Promise((resolve, reject) => {
        ...
        const js = `window["${id}"]("${roundTripId}", "${fn}", ${JSON.stringify(
          params
        )});`;
  ...
  const onMessage = useCallback(
    async ({ nativeEvent: { data } }) => {
      try {
        ...
        if (type === "WCQRModalClosedEvent") {
          ...
          if (!roundTrip) {
            return Promise.reject(
              new Error(`Encountered synchronization error!`)
            );
          }

          setRoundTrips(
            Object.fromEntries(
              Object.entries(roundTrips).filter(([k]) => k !== id)
            )
          );
        ...
        return Promise.reject(
          new Error(`Encountered unexpected type, ${type}.`)
        );
      } catch (e) {
        /* web */
      }
